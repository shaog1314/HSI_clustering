function [lambda, EV]=powerMethodForSPRING(U, V, W, data, param, whichVar)
...
%non-convex optimization. arXiv preprint 2020, arXiv: 2002.12266)
%for the SPRING Methods together with the SGD estimator. We consider the
%NMF problem
...
%     N = size(V, 2);
    ...
    switch whichVar
        case 'U' %In this case, we have to run the power method two times.
            ...
            %Save the batch size via V (which corresponds to Vtilde in ONMFTV_SPRING_SGD_Fernsel
            batchSize = size(V,2);
            xi = randn(K, 1);
            xi = xi./norm(xi);
            WW = batchSize/size(data.X,2) * ( param.sigma1 * (W'*W) + param.sigma2*eye(K) );
            for i=1:param.powerIt
                x_tmp = WW*xi;
                xi = x_tmp./norm(x_tmp);
            end
