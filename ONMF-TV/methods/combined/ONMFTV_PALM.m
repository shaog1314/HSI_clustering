function [UTV,V,clusterQuality,timeNeeded]=ONMFTV_PALM(pathToSaveResults, numClusters, tauTV, sigma1, sigma2, iniType, indexParam, whichData, dataPath)
%Computes the orthogonal NMF with orthogonal and TV penalty terms based 
%on the PALM update rules in Driggs et al. 2020.
...
%         end
        ...
        grad_U = U*(V*V') - X*V' + param.sigma1 * (W*(W'*U) - W) +...
            param.sigma2 * (U - W); %Calculate the gradient
        ...
        eta_V = 1/L_V; %Calculate the step size of the gradient descent step
        grad_V = (U'*U)*V - U'*X; %Calculate the gradient
        ...
        eta_W = 1/L_W; %Calculate the step size of the gradient descent step
        grad_W = param.sigma1 *(U*(U'*W) - U) + param.sigma2*(W - U); %Calculate the gradient
