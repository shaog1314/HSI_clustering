function [kappa, AA,OA,ACC,grp] = fast_performance(gnd,idx)
...
grp1=grp;
grp1(gnd==0)=0;
[kappa, AA,OA,ACC] = evaluate_results(grp1(:), gnd(:));
    ...
    function [kappa, acc_A,acc_O,acc] = evaluate_results(gtest, grd)
        if sum(grd==0)~=0
            c=max(grd)-min(grd);
            CM1=confusionmat(gtest,double(grd));
            CM=CM1(2:end,2:end);
            d=sum(diag(CM));
